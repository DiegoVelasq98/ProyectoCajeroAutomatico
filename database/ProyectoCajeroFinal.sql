...
BEGIN
    IF INSERTING THEN
        v_operacion := 'INSERT';
        v_detalle := 'Nueva transacción: ' || :NEW.tipo_transaccion || ', Monto: ' || :NEW.monto;
    ELSIF UPDATING THEN
        v_operacion := 'UPDATE';
        v_detalle := 'Cambio en transacción: de ' || :OLD.monto || ' a ' || :NEW.monto;
    ELSIF DELETING THEN
        v_operacion := 'DELETE';
        v_detalle := 'Transacción eliminada: ' || :OLD.tipo_transaccion || ', Monto: ' || :OLD.monto;
    END IF;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON prestamo TO C##Dev2;
GRANT SELECT, INSERT, UPDATE, DELETE ON prestamo TO C##Dev3;
